...
class WPCF7_Editor {
	...
	public function notice( $id, $panel ) {
		$section = preg_replace( '/-panel$/', '', $id, 1 );

		if ( wpcf7_validate_configuration() ) {
			$config_validator = new WPCF7_ConfigValidator( $this->contact_form );
			$count_errors = $config_validator->count_errors( array(
				'section' => $section ) );

			if ( $count_errors ) {
				$error_notice = sprintf(
					_n(
						'%s configuration error found in this tab panel',
						'%s configuration errors found in this tab panel',
