...
class WPCF7_ConfigValidator {
	...
	public function get_doc_link( $error_code = '' ) {
		$base_url = __( 'https://contactform7.com/configuration-errors/',
			'contact-form-7' );

		$error_code = strtr( $error_code, '_', '-' );

		$url = sprintf( '%s/%s',
			untrailingslashit( $base_url ), $error_code );
		...
		&& ! wpcf7_is_email_in_site_domain( $sender ) ) {
			$this->add_error( sprintf( '%s.sender', $template ),
				self::error_email_not_in_site_domain, array(
					'link' => $this->get_doc_link( 'email_not_in_site_domain' ) ) );
		}
		...
		if ( $invalid_mail_header_exists ) {
			$this->add_error( sprintf( '%s.additional_headers', $template ),
				self::error_invalid_mail_header, array(
					'link' => $this->get_doc_link( 'invalid_mail_header' ) ) );
		}
	...
	public function detect_invalid_mailbox_syntax( $section, $content, $args = '' ) {
		$args = wp_parse_args( $args, array(
			'link' => $this->get_doc_link( 'invalid_mailbox_syntax' ),
			'message' => '',
			'params' => array() ) );
		...
		if ( '' === $content ) {
			return $this->add_error( $section,
				self::error_maybe_empty, array(
					'link' => $this->get_doc_link( 'maybe_empty' ) ) );
		}
		...
		if ( ! @is_readable( $path ) || ! @is_file( $path ) ) {
			return $this->add_error( $section,
				...
				array(
					...
					'link' => $this->get_doc_link( 'file_not_found' ) ) );
		}
	...
	public function detect_html_in_message( $section, $content ) {
		...
		if ( $stripped != $content ) {
			return $this->add_error( $section,
				self::error_html_in_message, array(
					'link' => $this->get_doc_link( 'html_in_message' ) ) );
		}
